	    ...
	    local sample_name="$1"
		    ...
		    # # Remove duplicates
			    ...
			    # Trim using Trimmomatic
				    ...
				    # Map using hisat2
				        $hisat2 -p 40 -x $genome_index -1 "$trim_output_dir/${sample_name}_1_uniq_paired.fastq.gz" -2 "$trim_output_dir/${sample_name}_2_uniq_paired.fastq.gz" -S "$bam_output_dir/${sample_name}.sam"

						samtools view -bS "$bam_output_dir/${sample_name}.sam" > "$bam_output_dir/${sample_name}.bam"
                        # rm "$bam_output_dir/${sample_name}.sam"
